...
var App = {
  ...
  build: function build(options) {
    ...
    options.staticDir = options.staticDir ||
                        path.join(__dirname, '..', 'static');
    options.assertionDir = options.assertionDir ||
                           path.join(__dirname, '..', 'assertions');

    app.staticDir = options.staticDir;
    app.assertionDir = options.assertionDir;
    ...
    app.use(function(req, res, next) {
      ...
      var baked = res.locals.baked = (type === 'baked');
      var signed = res.locals.signed = (type === 'signed');
      ...
      var sets, merges;
      try {
        sets = JSON.parse(setStr);
        merges = JSON.parse(mergeStr);
      }
      catch (e) {
        return next(e);
      }
      res.finalize = function(data) {
        ...
        if (signed) {
          ...
          var url = res.locals.self.replace(/&type=\w+|type=\w+&?/i, '');
          // TODO: use image(s) in result, or otherwise allow configurable bake image
          var png = fs.createReadStream(path.join(__dirname, '../static/badge-under-256k.png'));
          return bakery.bake({
            image: png,
            data: url
          }, function(err, imageData) {
            if (err) next(err);
            return res.render('baked.html', {
              dataURI: 'data:image/png;base64,' + imageData.toString('base64'),
              assertionUrl: url
            });
          });
        }
    ...
    if (options.index) {
      app.get('/', function(req, res, next) {
        fs.readdir(options.assertionDir, function(err, files) {
          // log error here, but don't pass to next()
          if (files) 
    ...
    app.url = function(file, options) {
      return app.baseURL + app.path(file, options);
    };
    ...
    app.listen = function(port, cb) {
      ...
      app._listen(port, function() {
        app.baseURL = 'http://127.0.0.1:' + this.address().port;
        if (cb) cb.call(this);
      });
    };
